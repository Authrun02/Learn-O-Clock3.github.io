...
function renderTimeline() {
    ...
    daySessions.forEach(session => {
        ...
        const subjectColors = {
            'Mathematics': { bg: '#EDE9FE', border: '#8B7FD4', icon: 'fa-calculator' },
            'Physics': { bg: '#EDE9FE', border: '#A99DE6', icon: 'fa-atom' },
            'Coding': { bg: '#FEF3C7', border: '#F4C430', icon: 'fa-code' },
            'Languages': { bg: '#DBEAFE', border: '#3B82F6', icon: 'fa-language' }
        };
        const colors = subjectColors[session.subject] || subjectColors['Mathematics'];

        html += `
            <div class="session-event ${isPast ? 'past' : ''} ${isLive ? 'live' : ''}" 
                 style="top: ${top}px; height: ${Math.max(height, 50)}px; border-left-color: ${colors.border}; background: ${colors.bg};"
...
function openSessionDetails(sessionId) {
    ...
    document.getElementById('details-icon').style.background = subjectColors[session.subject] || '#8B7FD4';
...
function createSessionCard(session, isActive) {
    const date = new Date(session.date);
    const subjectColors = {
        'Mathematics': { bg: '#EDE9FE', border: '#8B7FD4', icon: 'fa-calculator', badge: 'math' },
        'Physics': { bg: '#EDE9FE', border: '#A99DE6', icon: 'fa-atom', badge: 'physics' },
        'Coding': { bg: '#FEF3C7', border: '#F4C430', icon: 'fa-code', badge: 'coding' },
        'Languages': { bg: '#DBEAFE', border: '#3B82F6', icon: 'fa-language', badge: 'languages' }
    };
    const colors = subjectColors[session.subject] || subjectColors['Mathematics'];
